...
export default function UserTable({ users, currentUserRole, onStatusChange, onRoleChange, onDeleteUser }: UserTableProps) {
  ...
  const getRoleColor = (role: UserRole) => {
    switch (role) {
      ...
      case 'co-admin': return 'bg-purple-100 dark:bg-purple-900 text-purple-800 dark:text-purple-200';
  ...
  const canModifyUser = (userRole: UserRole) => {
    ...
    if (currentUserRole === 'co-admin' && userRole === 'driver') return true;
    return false;
  };
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                {canModifyUser(user.role) ? (
                  <select
                    ...
                    className="text-xs px-2 py-1 rounded-full border-0 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500"
                  >
                  ...
                  <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getRoleColor(user.role)}`}>
                    {user.role === 'co-admin' ? 'Co-Admin' : user.role.charAt(0).toUpperCase() + user.role.slice(1)}
                  </span>
                )}
              </td>
              <td className="px-6 py-4 whitespace-nowrap">
                {canModifyUser(user.role) ? (
                  <select
                    value={user.status}
                    onChange={(e) => onStatusChange(user.userId, e.target.value as User['status'])}
                    className="text-xs px-2 py-1 rounded-full border-0 dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500"
                  >
